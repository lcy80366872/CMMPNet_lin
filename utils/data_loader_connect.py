...
class ImageGPSDataset(data.Dataset):
    def __init__(self, image_list, sat_root, mask_root, gps_root,connect_root_1,connect_root_2,sat_suffix="png", mask_suffix="png", pgs_suffix="jpg",
        ...
        self.connect_root_1 = connect_root_1
        self.connect_root_2 = connect_root_2
    ...
    def _concat_images(self, image1, image2):
        #注意这里的图像格式还不是tensor格式，所以第三维度是通道
        if image1 is not None and image2 is not None:
            img = np.concatenate([image1, image2], 2)
        elif image1 is None and image2 is not None:
            img = image2
    ...
    def _data_augmentation(self, sample):
        #ndim代表的是其维度，即这个数组是几维的，相当于给gps信息加了通道这一第三维度，便于后续合并
        if sample['gps'].ndim == 2:
        ...
        if self.randomize:
            ...
            img = self._concat_images(sample['image'], gps)
    ...
    def __getitem__(self, index):
        image_id = self.image_list[index]
        sample = self._read_data(image_id)
        img, mask = self._data_augmentation(sample)
        img, mask = torch.Tensor(img), torch.Tensor(mask)
        return img, mask
