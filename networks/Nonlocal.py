...
class NLBlockND_Fuse(nn.Module):
    def __init__(self, in_channels, inter_channels=None, mode='embedded',
                 dimension=3, bn_layer=True):
        """Implementation of Non-Local Block with 4 different pairwise functions but doesn't include subsampling trick
        args:
            ...
            bn_layer: whether to add batch norm
        """
        super(NLBlockND_Fuse, self).__init__()
        ...
        # function g in the paper which goes through conv. with kernel size 1
        self.g = conv_nd(in_channels=2*self.in_channels, out_channels=self.inter_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # this reshaping and permutation is from the spacetime_nonlocal function in the original Caffe2 implementation
        v=torch.cat((x, y), 1)
        g_x = self.g(v).view(batch_size, self.inter_channels, -1)
        g_x = g_x.permute(0, 2, 1)
